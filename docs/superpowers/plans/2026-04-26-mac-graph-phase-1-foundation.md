...
export async function runScip(repoDir: string): Promise<scip.Index> {
  ...
  await new Promise<void>((resolve, reject) => {
    const proc = spawn('npx', ['scip-typescript', 'index', '--cwd', repoDir, '--output', out], {
      stdio: ['ignore', 'pipe', 'pipe']
    })
    let stderr = ''
    proc.stderr.on('data', d => { stderr += d.toString() })
    proc.on('close', code => code === 0 ? resolve() : reject(new Error(`scip-typescript exit ${code}: ${stderr}`)))
  })
...
export function parseScipIndex(idx: scip.Index, repoDir: string): ParsedScip {
  const symbols: SymbolNode[] = []
  const references: ReferenceEdge[] = []
  const fileSymbols = new Map<string, string[]>()

  for (const doc of idx.documents) {
    const filePath = doc.relative_path
    const ids: string[] = []
    for (const sym of doc.symbols) {
      const node: SymbolNode = {
        ...
        signature: sym.signature_documentation?.text ?? '',
    ...
    for (const occ of doc.occurrences) {
      // Range is [startLine, startCol, endLine, endCol]; SCIP uses 0-based
      const [sl, sc, el, ec] = readRange(occ.range)
      const owner = symbols.find(s => s.id === occ.symbol && s.filePath === filePath)
      if (owner && (occ.symbol_roles & scip.SymbolRole.Definition)) {
        owner.startLine = sl + 1; owner.startCol = sc
        owner.endLine = el + 1; owner.endCol = ec
      }
      // Reference occurrences: connect from the enclosing symbol (first symbol in doc that contains the occurrence range)
      // and emit a REFERENCES edge if not a definition.
      if ((occ.symbol_roles & scip.SymbolRole.Definition) === 0) {
        const enclosing = findEnclosingSymbol(doc, sl)
        if (enclosing && enclosing !== occ.symbol) {
          references.push({
            fromSymbolId: enclosing,
            toSymbolId: occ.symbol,
            kind: refKindFromRoles(occ.symbol_roles, occ.syntax_kind),
            refLine: sl + 1,
            refCol: sc
          })
        }
...
function readRange(range: number[]): [number, number, number, number] {
  // SCIP encoding: [startLine, startCol, endCol] when single-line, else [startLine, startCol, endLine, endCol]
  if (range.length === 3) return [range[0]!, range[1]!, range[0]!, range[2]!]
  return [range[0]!, range[1]!, range[2]!, range[3]!]
}

function findEnclosingSymbol(doc: scip.Document, line: number): string | null {
  for (const occ of doc.occurrences) {
    if ((occ.symbol_roles & scip.SymbolRole.Definition) === 0) continue
    const [sl, , el] = readRange(occ.range)
    if (sl <= line && line <= el) return occ.symbol
  }
  return null
}

function refKindFromRoles(roles: number, _syntax: scip.SyntaxKind): RefKind {
  if (roles & scip.SymbolRole.WriteAccess) return 'write'
  if (roles & scip.SymbolRole.ReadAccess) return 'read'
  // SCIP doesn't distinguish call vs type-ref directly; default to 'call'.
  // Phase 2 can refine using syntax_kind heuristics.
  return 'call'
}

function mapKind(k: scip.SymbolInformation_Kind): SymbolNode['kind'] {
  switch (k) {
    case scip.SymbolInformation_Kind.Function: return 'function'
    case scip.SymbolInformation_Kind.Method: return 'method'
    case scip.SymbolInformation_Kind.Class: return 'class'
    case scip.SymbolInformation_Kind.Interface: return 'interface'
    case scip.SymbolInformation_Kind.TypeAlias:
    case scip.SymbolInformation_Kind.Type: return 'type'
